...
package com.lujianfei.icecontroller.socket;
...
public class TcpSocket {
	...
	TcpSocketListener _TcpSocketListener = null;
	...
	public TcpSocket(IpAddress ipaddr, int port,TcpSocketListener mTcpSocketListener) throws java.io.IOException {
//		socket = new Socket(ipaddr.getInetAddress(), port); modified
		socket = new Socket();
		_TcpSocketListener = mTcpSocketListener;
		...
		try {
			...
			if(_TcpSocketListener!=null){
			_TcpSocketListener.onConnectSuccesfull(ipaddr.toString(), port);
			}
		} catch (java.io.IOException e) {
			if(_TcpSocketListener!=null){
				_TcpSocketListener.onConnectFailed();
		    }
			lock = false;
			throw e;
		}
